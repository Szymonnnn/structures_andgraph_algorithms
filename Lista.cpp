...
template <class T>
T* moja_lista<T>::getHead() {
	return this->pierwszy;
}

template <class T>
T* moja_lista<T>::getTail() {
	return this->ostatni;
}
...
template <class T>
T* moja_lista<T>::get(int index) {
	if (index == 0) {
		return this->pierwszy;
	} else if (index == this->rozm_listy - 1) {
		return this->ostatni;
	} else if (index < 0 || index >= this->rozm_listy) {
		return NULL;
	}
	if (index < this->rozm_listy / 2) {
		T* temp = this->pierwszy;
		int i = 0;
		while (temp) {
